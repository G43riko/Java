...
public class ListO {
		...
		for(p_act = p_pred = p_prvy; p_act.getP_dalsi()!=null ; p_pred=p_act,p_act=p_act.getP_dalsi()){
			if(p_act.getKey() == key){
				if(p_act==p_prvy){
					...
					num--;
					vypis.log("uspešne sa vymazal "+p_act.getValue());
					return;
				}
	...
	public void set(int key, String value){
		...
		if(find!=null){
			vypis.log(find.getValue()+" sa uspešne prepísal na "+value);
			find.setValue(value);
			return;
		}
		vypis.log(value+" sa nepodarilo nájs");
	}
	...
	private Node getNode(int key){
		...
		vypis.log("lutujeme ale "+key+" sa v zozname nenachádza");
		return null;
	}
	...
	public void add(int key, String value){
		...
		if(p_prvy == null){
			...
			p_pred.setP_dalsi(p_novy);
			vypis.log(value+" bol úspešne pridaný");
		}
	}
