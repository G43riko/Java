...
//	private static final int VERTEX_COUNT = 128;
	...
	public Terrain(float gridX, float gridZ, String heighMap){
		this.x = gridX * SIZE;
		this.z = gridZ * SIZE;
		this.model = generateTerrain( heighMap);
	}
	
	public Terrain(float gridX, float gridZ,  float[][] map){
		this.x = gridX * SIZE;
		this.z = gridZ * SIZE;
		this.model = generateTerrain( map);
	}
	...
	public Terrain(float gridX, float gridZ, int type){
		this.x = gridX * SIZE;
		this.z = gridZ * SIZE;
		switch(type){
			case PERLIN_NOISE:
				this.model = generateTerrain(  PerlinNoise.GeneratePerlinNoise(PerlinNoise.generateWhiteNoise(SIZE, SIZE), 8, 0.7f, true));
				break;
			case SIMPLEX_NOISE:
				this.model = generateTerrain( SimplexNoise.generateOctavedSimplexNoise(SIZE, SIZE, 6, 0.8f, 0.008f));
				break;
			case PROCEDURAL_TERRAIN:
				this.model = generateTerrain( ProceduralTerrain.create(SIZE, SIZE));
				break;
			default:
				this.model = generateTerrain( SimplexNoise.generateSimplexNoise(SIZE, SIZE));
		}
	}
	...
	private Model generateTerrain(String heightMap){
		...
		try {
			...
			GLog.write("nepodarilo sa naËÌtaù s˙bor: res/textures/" + heightMap + ".png", e);
		}
