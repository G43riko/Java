...
public class ProceduralTerrain {
	...
	private static void generateMap(int x1, int y1, int x2, int y2) {
		...
		mapa[b1.getXi()][b1.getYi()] =  average(cl1, cl2);
				
		GVector2f b2 = new GVector2f((x1 + x2) / 2, y2);
		mapa[b2.getXi()][b2.getYi()] =  average(cl3, cl4);
		
		GVector2f b3 = new GVector2f(x1, (y1 + y2) / 2);
		mapa[b3.getXi()][b3.getYi()] =  average(cl1, cl3);
		
		GVector2f b4 = new GVector2f(x2, (y1 + y2) / 2);
		mapa[b4.getXi()][b4.getYi()] =  average(cl2, cl4);
		
		GVector2f b5 = new GVector2f((x1 + x2) / 2, (y1 + y2) / 2);
//		mapa[b5.getXi()][b5.getYi()] =  ff(x1, y1, x2, y2);
		mapa[b5.getXi()][b5.getYi()] =  average(cl1, cl2, cl3, cl4) + (float)(Math.random()-0.5);
